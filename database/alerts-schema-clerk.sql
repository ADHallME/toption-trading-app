-- Alert System Database Schema (CLERK-COMPATIBLE VERSION)
...
CREATE TABLE IF NOT EXISTS alert_criteria (
  ...
  user_id TEXT NOT NULL, -- Changed from UUID to TEXT for Clerk user IDs (e.g., "user_abc123")
  name TEXT NOT NULL,
...
-- RLS Policies (Note: You'll need to configure Supabase to validate Clerk JWT tokens)
...
-- 2. Add Clerk's JWT issuer URL to "Additional JWT Issuers"
-- 3. Clerk user IDs are strings like "user_2abc123def", not UUIDs
-- 4. Make sure your Clerk JWT includes the user_id in the 'sub' claim
